...
ansible-playbook -e rm_dest=<destination for modules and module utils> \
                 -e test_dest=<destination for test directory> \
                 -e structure=role \
                 -e model=<model> \
                 site.yml
```
...
- `test_dest`: The directory in which the tests structure will be created
...
ansible-playbook -e rm_dest=~/github/rm_example \
                 -e test_dest=~/github/rm_example/tests \
                 -e structure=collection \
...
ansible-playbook -e rm_dest=./rmb_tests/roles/my_role \
                 -e test_dest=./test/integration/targets \
                 -e structure=role \
                 -e model=models/myos/interfaces/myos_interfaces.yml \
                 site.yml
```
